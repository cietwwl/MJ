                  ...
                  <div class="col-lg-12">
                     <section class="panel">
                        ...
                        <div class="panel-body">
                           <form class="form-inline" role="form" action="UserServlet?method=chargeRecord" method="post" >
                           	  <input type="hidden" name="index" value="1"/>
                              <div class="form-group">
                                 <label  for="exampleInputEmail2">代理ID/姓名</label>
                                 <input type="text" class="form-control" id="exampleInputEmail2" placeholder="代理ID/姓名" name="operateIdOrName" value="${operateIdOrName}">
                              </div>
                              
                              <div class="form-group">
                              	<label for="registDate">房卡数量</label>
                                <input type="radio" name="roomCard" value="asc"  <c:if test="${roomCard eq 'asc'}">checked="checked"</c:if>>升序
								<input type="radio" name="roomCard" value="desc" <c:if test="${roomCard eq 'desc'}"> checked="checked"</c:if>>降序
								<input type="radio" name="roomCard" value="" <c:if test="${roomCard eq ''}"> checked="checked"</c:if>>默认
                              </div>
                              
                              <div class="form-group">
                              	<label for="registDate">消耗数量</label>
                              ...
                              <div class="form-group">
                                ...
                                <input type="radio" name="chargeTotal" value="asc"  <c:if test="${chargeTotal eq 'asc'}">checked="checked"</c:if>>升序
								<input type="radio" name="chargeTotal" value="desc" <c:if test="${chargeTotal eq 'desc'}">checked="checked"</c:if>>降序
								<input type="radio" name="chargeTotal" value="" <c:if test="${chargeTotal eq ''}">checked="checked"</c:if>>默认
                              </div>
                              ...
                              <table class="display table table-bordered table-striped" id="example">
                                 <thead>
                                    <tr>
                                       ...
                                       <th class="hidden-phone">消耗数量</th>
                                       <th class="hidden-phone">充值总数</th>
                                    </tr>
                                 </thead>
                                 <tbody>
                                 	<c:forEach items="${pageUtil.list}" var="list" varStatus="vs">
							    		<tr class="gradeX">
							    			<td>${list.id }</td>
							    			<td>${list.userName }</td>
							    			<td class="hidden-phone">${list.roomCard }</td>
							    			<td class="hidden-phone">${list.consumeCard }</td>
							    			<td class="hidden-phone">${list.chargeTotal }</td>
							    		</tr>
					    			</c:forEach>
                                 </tbody>
                              </table>

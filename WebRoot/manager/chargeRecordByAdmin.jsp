                     ...
                     <!-- 充值查询 -->
                      <section class="panel">
                        ...
                        <div class="panel-body">
                           <form class="form-inline" role="form" action="UserServlet?method=chargeRecordByAdmin" method="post">
                           	  <input type="hidden" name="index" value="1"/>
                              <div class="form-group">
                                 <label  for="exampleInputEmail2">代理Id/用户名</label>
                                 <input type="text" class="form-control" id="exampleInputEmail2" placeholder="代理Id/用户名" name="idOrNick" value="${idOrNick}">
                              </div>
	                          <div class="form-group">
                              ...
                              <div class="form-group">
                              	<label for="registDate">充值时间</label>
                                <input type="radio" name="chargeDate" value="asc"  <c:if test="${chargeDate eq 'asc'}">checked="checked"</c:if>>升序
								<input type="radio" name="chargeDate" value="desc" <c:if test="${chargeDate eq 'desc'}">checked="checked"</c:if>>降序
								<input type="radio" name="chargeDate" value="" <c:if test="${chargeDate eq ''}">checked="checked"</c:if>>默认
                              </div>
                              ...
                              <table class="display table table-bordered table-striped" id="example">
                                 <thead>
                                    <tr>
                                       ...
                                       <th>代理名字</th>
                                       <th>代理电话</th>
                                       <th class="hidden-phone">充值数量</th>
                                       <th class="hidden-phone">赠送数量</th>
                                       <th class="hidden-phone">充值时间</th>
                                    </tr>
                                 </thead>
                                 <tbody>
                                 	<c:forEach items="${pageUtil.list}" var="list" varStatus="vs">
							    		<tr class="gradeX">
							    			<td>${list.operateId }</td>
							    			<td>${list.userName }</td>
							    			<td>${list.mobile }</td>
							    			<td class="hidden-phone">${list.roomCard }</td>
							    			<td class="hidden-phone">${list.sendCard }</td>

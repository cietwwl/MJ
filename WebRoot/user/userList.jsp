                  ...
                  <div class="col-lg-12">
                     <section class="panel">
                        ...
                        <div class="panel-body">
                           <form class="form-inline" role="form" action="UserServlet?method=userList" method="post">
                           	  <input type="hidden" name="index" value="1"/>
                              <div class="form-group">
                                 <label  for="exampleInputEmail2">Id/用户名</label>
                                 <input type="text" class="form-control" id="exampleInputEmail2" placeholder="Id/用户名" name="idOrNick" value="${idOrNick}">
                              </div>
                              
                              <div class="form-group">
                                 <label  for="exampleInputPassword2">推荐号</label>
                                 <input type="text" class="form-control" id="exampleInputPassword2" placeholder="推荐号" name="recommendId" value="${recommendId}">
                              </div>
                              
	                          <div class="form-group">
			                      <label for="beginDate">注册开始</label>
		                          <input type="date" class="form-control" name="beginDate" value="${beginDate }">
			                      <label for="endDate">注册结束</label>
                              ...
                              <table class="display table table-bordered table-striped" id="example">
                                 <thead>
                                    <tr>
                                       <th>用户ID</th>
                                       <th>昵称</th>
                                       <th class="hidden-phone">省份</th>
                                       <th class="hidden-phone">城市</th>
                                       <th class="hidden-phone">房卡</th>
                                       <th class="hidden-phone">消耗</th>
                                       <th class="hidden-phone">注册</th>
                                 ...
                                 <tbody>
                                 	<c:forEach items="${pageUtil.list}" var="user" varStatus="vs">
							    		<tr class="gradeX">
							    			<td>${user.id }</td>
							    			<td>${user.nickname }</td>
							    			<td class="hidden-phone">${user.province }</td>
							    			<td class="hidden-phone">${user.city }</td>
							    			<td class="hidden-phone">${user.roomcard }</td>
							    			<td class="hidden-phone">${user.consumeCard }</td>
							    			<td class="hidden-phone"><fmt:formatDate value="${user.createdate}"  pattern="yyyy-MM-dd HH:mm:ss" /></td>
							    			<td class="hidden-phone">
							    			<c:if test="${user.recommendId!=0 }">${user.recommendId }</c:if>
							    			</td>
							    		</tr>
					    			</c:forEach>
                                 </tbody>
